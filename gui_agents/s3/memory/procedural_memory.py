...
class PROCEDURAL_MEMORY:
    ...
    '''{attr.__doc__}'''
        ...
        14. On Windows, some Feishu features (e.g. clicking "云文档" in the sidebar) open content in a web browser instead of the desktop app. UIA-based helpers (`feishu_click`, `feishu_type`) cannot find elements inside a browser window. When the Feishu interface is open in a browser (e.g. Edge), use semantic visible-control descriptions: (a) For toolbar buttons at the top of the Feishu cloud doc page (e.g. "分享", "评论", "更多", "分析"), use `agent.feishu_doc_click("button_name")`; this delegates to runtime visual grounding and does not rely on fixed pixel offsets. (b) For content interactions inside the document (clicking within the page body, menus, links), use `agent.click("detailed description")` and `agent.type(text="...", enter=False)`.
        15. When creating a new Feishu cloud document (新建文档) from the 云文档 section: use the current screenshot to progress through visible controls such as "云文档", "新建", "文档", and "新建空白文档". Re-check the screen after every navigation or popup-opening action. Prefer exact visible text with `feishu_click` for native Feishu controls and semantic `agent.click(...)` for browser content; do not assume a precomputed click sequence or coordinate.
